...
void Maestro::Init() {
    ...
    if (restart_file.empty()) {
        ...
        for (int lev = 0; lev <= finest_level; ++lev) {
            ...
            unew[lev].setVal(0.);
            snew[lev].setVal(0.);
            // copy the old temperature into the corrected temperature
            // may be needed for the initial projection with thermal diffusion
            MultiFab::Copy(TempC[lev], sold[lev], Temp, 0, 1, 1);
        }
